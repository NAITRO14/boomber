...
class ForRules : public Fl_Box
{
public:
	ForRules(int X, int Y, int W, int H, const char* L = 0)
		: Fl_Box(X, Y, W, H, L)
	{
		align(FL_ALIGN_INSIDE | FL_ALIGN_LEFT);
		labelfont(FL_HELVETICA);
...
int main(int argc, char** argv)
{
	...
	ForRules rt4(85, 110, 815, 119, "После каждой игры, вы можете получить очки, которые влияют на положение в таблице\nлидеров, но, чем дольше идет игра, тем меньше очков вы получите за игру.\nЗарабатывая больше очков, вы продвигаетесь выше по таблице. Обратите внимание,\nчто за игры, где был включен \"Чит\", очки не насчитываются. Таблица лидеров\nсуществует в пределах одного приложения(не мультиплеер).");

	ForRules rt5(85, 240, 815, 131, "Когда вы зайдете в игру, вы увидите показатели \"мин осталось\" и \"мин найдено\".\nЭти показатели изменяются, когда вы помечаете клетку флажком(правая кнопка мыши),\nно учтите, что этоти индикаторы отображают только количество помеченных флажком\nклеток. Эти цифры не обозначают реально найденое количество мин.");
